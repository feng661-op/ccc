...
out={'protocol':{'selection':'7-day load seasonal-naive selected using January only; formal period is report-only; K=3/scenario settings remain predeclared and are not retuned on formal period','formal_period':'2025-02-01..2025-12-31','no_formal_retuning':True}}
out['load_forecast']={'january_selection':load_benchmark(7,31),'formal_generalization':load_benchmark(31,365)}
...
out['scenario_structure']={'K1_max_point_error_kwh':max(point_err),'representative_K3':diag,'max_weight_sum_error':max(abs(x['weight_sum']-1) for x in diag),'max_clip_rate':max(x['clip_rate'] for x in diag)}
...
out['scenario_count']={'K1_point':metrics['D_point'],'K3_main':metrics['D'],'K5':runs['D_K5'],'interpretation':'formal-period realized-policy stability only; K=3 was predeclared and is not selected by these formal results; not theoretical VSS/VOI'}
# terminal value sensitivity is intentionally limited to January to avoid formal-period retuning.
jan={}
for tv in (.25,.45,.75):
    print('JAN terminal',tv,flush=True);t0=time.time();s=simulate_strategy(data,name=f'Jan_vT_{tv}',use_new_vintage=True,allow_revision=True,scenario_count=3,end_day=31,verbose=False,terminal_value=tv)
