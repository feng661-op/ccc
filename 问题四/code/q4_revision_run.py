...
from q4_sim import SimConfig,SimState,simulate_range as simulate_native,replay_summary
...
def simulate_range(data,pf,cfg,*args,**kwargs):
    if cfg.settlement_clock=='adjustment_time':
        from q4_semantics_fast import simulate_range as exact_event_time
        return exact_event_time(data,pf,cfg,*args,**kwargs)
    return simulate_native(data,pf,cfg,*args,**kwargs)

def semantic_adapter_hash(cfg):
    return sha(CODE/'q4_semantics_fast.py') if cfg.settlement_clock=='adjustment_time' else None
...
def provenance(cfg):
    ...
    return {'backend':VERSION,'base_q3_head':protected['base_q3_head'],
        ...
        'semantic_adapter_sha256':semantic_adapter_hash(cfg),'runner_sha256':sha(Path(__file__)),
...
def formal(branch,clock='delivery',mode='selected'):
    ...
    if clock=='adjustment_time' and dec['provenance'].get('semantic_adapter_sha256')!=semantic_adapter_hash(cfg):raise AssertionError('semantic solver changed after January freeze')
    if mode=='backup':
