...
def solve_intraday(bundle,prices,soc0,B,A0,lead,eh,cfg,event_price=None,fixed_contract=None):
    ...
    for j,ix in q.items():
        t=j+1-start;b=float(B[j]); groups=[(0,1.0)]
        ...
        mean_p=float(np.dot(bundle.weights,prices[:,t]))
        nonconvex=event_clock and b>0 and mean_p>2*float(event_price)+1e-12
        binary=None
        if nonconvex:
        ...
        for k,w in groups:
            p=mean_p;pe=float(event_price) if event_clock else p
            phi=add(w)
    ...
    if any(integers):
        ...
        sol=milp(cost,integrality=np.asarray(integers),bounds=Bounds(lb,upper),constraints=LinearConstraint(mat,lo,hi),options={'mip_rel_gap':1e-4,'time_limit':60})
    else:
