...
def solve_intraday(bundle,prices,soc0,B,A0,lead,eh,cfg,event_price=None,fixed_contract=None):
    ...
    for j,ix in q.items():
        ...
        if event_clock and b>0 and p>2*pe+1e-12:
            # Exact disaggregated two-segment formulation. Binary integrality
            # is unchanged; its LP relaxation is the convex hull of the two
            # bounded fee segments, not a loose big-M epigraph relaxation.
            m0,b0=lines[0];m1,b1=lines[1]
            z=add(b1-b0,bound=(0,1),integer=True)
            lo=add(m0,bound=(0,b));hi=add(m1,bound=(0,U));cash_offset+=b0
            eqs.append({ix:1,lo:-1,hi:-1});rhs.append(0.)
            ubs.extend(({lo:1,z:b},{hi:-1,z:b},{hi:1,z:-U}));urhs.extend((b,0.,0.))
